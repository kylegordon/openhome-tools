...
- **Disband** (`songcast_disband.py`): auto-detect each device's role (sender/receiver/standalone) by probing `Receiver`/`Sender` state → for receivers: `Receiver.SetSender(Uri="", Metadata="")` (clears "zombie" receivers still bound to a dead `ohz://` URI — `Receiver.Stop` alone is not sufficient) → `Receiver.Stop` → `Product.SetSourceIndex` back to a non-Songcast source (e.g. Playlist) → for senders: `Playlist.Stop`. Verifies final standalone state afterward.
