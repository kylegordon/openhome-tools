...
- **A device waking from standby responds slowly, and `SetStandby(False)` can time out while still succeeding.** Observed on a Selekt: the wake call timed out, yet the device woke and every subsequent call worked. The same device is fast once already awake. So a failed wake must NOT abort a grouping run — `songcast_group.py` deliberately ignores `wake_device()`'s return value, and treating it as fatal would abandon joins that go on to succeed. Allow generous timeouts when a device may be in standby.
